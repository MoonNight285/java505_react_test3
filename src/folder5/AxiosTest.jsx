...
// axios : node.js 비동기 통신을 하기 위한 라이브러리, promise를 사용한다.
// axios는 rest 방식을 지원한다.
// get(요청주소, config) : get 방식으로 서버에게 데이터를 요청한다.
// post(요청주소, data[, ]) : post 방식으로 서버에게 데이터를 요청한다.
// put (url, data[, config]) : put 방식으로 서버에 데이터를 전달
// delete(url, data[, config]) : delete 방식으로 서버에 데이터 전달
// options(url, config) : axios 객체 생성 시 설정 정보를 변경할 수 있다.
// then() : 정상적으로 통신이 성공했을 경우 실행할 내용
// catch() : 통신이 실패했을 경우 실행

// async / await와 함께 사용 시 then()을 필요로 할경우 나중에 호출이 가능하다.
// async / await와 함께 사용 시 예외처리를 try/catch로 처리함

// 응답 객체 : 요청에 대한 응답 객체가 json 방식으로 전달된다.
// data{} : 서버에서 제공한 데이터 json 타입이다.
...
// 기본 사용방법이나 이 방법보다는 메소드를 이용하는 방식을 선호한다.
// axios({
...
// npm install axios 로 먼저 사용할 프로젝트에 axios를 설치
